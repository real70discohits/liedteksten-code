...
def build_sample_nwctxt(staff_names=("Bass", "Zang", "Ritme"),
    ...
    for name in staff_names:
        ...
        # --- Liedstart marker ---
        if include_liedstart and name in ("Bass", "Zang"):
...
# ===========================================================================
# Tests: _trim_staff_to_liedstart
# ===========================================================================
...
class TestTrimStaffToLiedstart:
    ...
    def test_trims_pickup_and_vooraf(self):
        ...
        result = self.concat._trim_staff_to_liedstart(lines)
    ...
    def test_no_liedstart_returns_unchanged(self):
        """Staff with Dur but no liedstart marker returns unchanged."""
        lines = [
            '|AddStaff|Name:"Bass"',
            '|Clef|Type:Bass',
            '|Note|Dur:4th|Pos:-2',
...
class TestCreatePrintSheet:
    ...
    def test_trims_vooraf_measures(self, sample_file, build_folder):
        """Vooraf measures should be removed from the output."""
        result = self.concat.create_print_sheet(sample_file, build_folder, "Test Song")
        content = result.read_text(encoding='utf-8')

        assert 'liedstart' in content
        whole_count = content.count('|Note|Dur:Whole')
    ...
    def test_no_liedstart_no_trim(self, build_folder):
        """If no liedstart marker, staffs should be kept intact (no crash)."""
